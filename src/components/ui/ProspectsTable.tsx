...
export default function ProspectsTable({ prospects, currentProfile }: { prospects: Prospect[]; currentProfile: Profile }) {
  ...
  async function saveNote() {
    ...
    setSaving(true)
    const supabase = createClient()
    await supabase.from('prospects').update({ notes: noteText, updated_at: new Date().toISOString() }).eq('id', selected.id)
    setSaving(false)
    setSelected(null)
    router.refresh()
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-3 items-center px-5 py-4 border-b border-gray-100">
        <input
          type="text" placeholder="Search name, IC, phone…" value={search}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-5 py-12 text-center text-gray-400">No prospects found</td></tr>
            )}
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-3">
                  <select
                    ...
                    className={`text-xs font-semibold px-2 py-1 rounded-full border-0 cursor-pointer ${STATUS_STYLES[p.status]}`}
                  >
                    {(Object.keys(STATUS_LABELS) as ProspectStatus[]).map(s => (
                      <option key={s} value={s}>{STATUS_LABELS[s]}</option>
                    ))}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-3 text-sm">
              {[
                ...
                ['Submitted',       new Date(selected.created_at).toLocaleString('en-MY')],
              ].map(([label, value]) => (
