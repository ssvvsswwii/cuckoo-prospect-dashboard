  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Total Leads',  value: total,     color: 'bg-brand' },
          { label: 'New',          value: newCount,  color: 'bg-blue-500' },
          { label: 'Converted',    value: converted, color: 'bg-green-500' },
        ].map(stat => (
          <div key={stat.label} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <p className="text-sm text-gray-500 font-medium">{stat.label}</p>
            <p className={`text-3xl font-extrabold mt-1 ${stat.color === 'bg-brand' ? 'text-brand' : stat.color === 'bg-blue-500' ? 'text-blue-600' : 'text-green-600'}`}>
              {stat.value}
            </p>
          </div>
        ))}
